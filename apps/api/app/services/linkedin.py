...
class LinkedInService:
    ...
    def __init__(self):
        ...
        if not self.client_id or not self.client_secret:
            logger.warning(
                "LinkedIn credentials not configured. Auto-sync will be disabled."
            )
    ...
    async def _transform_to_cv_profile(
        ...
        try:
            ...
            experience = self._extract_work_experience(
    ...
    def _extract_certifications(
        ...
        try:
            ...
            for cert in certs:
                try:
                    name = cert.get("name", "")
                    issuing_organization = cert.get("issuingOrganization", {}).get(
                        "name", ""
                    )

                    # Date handling
                    issue_date = self._parse_linkedin_date(cert.get("issueDate", {}))
                    expiry_date = self._parse_linkedin_date(
        ...
        try:
            ...
            for lang in language_data:
                try:
                    ...
                    proficiency = lang.get("proficiency", {}).get(
                        "level", "Intermediate"
                    )
